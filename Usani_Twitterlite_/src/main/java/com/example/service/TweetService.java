...
import com.example.model.TweetDisplay;
...
public class TweetService {
	...
	public List<TweetDisplay> findAllWithTag(String tag){
		List<Tweet> tweets = tweetRepository.findAllByUserOrderByCreatedAtDesc(tag);
		return formatTweets(tweets);
	}
	...
	private void addTagLinks(List<Tweet> tweets) {
		...
		for(Tweet tweet: tweets) {
			String message = (String) tweet.getMessage();
	...
	private List<TweetDisplay> formatTimestamps(List<Tweet> tweets){
		...
		for(Tweet tweet : tweets) {
			...
			tweetDisplay.setTags(tweet.getClass());
			...
			if (diff > 3) {
				...
				tweetDisplay.setDate(prettyTime.format(((Tweet) tweet).getCreatedAt()));
			}
